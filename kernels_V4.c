...
void smooth(int dim, pixel *src, pixel *dst) 
{
    ...
    int tmp = 0, sum_c_r1, sum_c_r2, sum_c_r3, sum_c_g1, sum_c_g2, sum_c_g3, sum_c_b1, sum_c_b2, sum_c_b3;
    int col_r1, col_r2,col_r3,col_g1,col_g2,col_g3,col_b1,col_b2,col_b3;
    for (i = 1; i < dim - 1; ++i) {
        tmp = i*dim;
        sum_c_r1 = src[RIDX(i-1,0,dim)].red + src[RIDX(i,0,dim)].red + src[RIDX(i+1,0,dim)].red;
        sum_c_r2 = src[RIDX(i-1,1,dim)].red + src[RIDX(i,1,dim)].red + src[RIDX(i+1,1,dim)].red;

        sum_c_g1 = src[RIDX(i-1,0,dim)].green + src[RIDX(i,0,dim)].green + src[RIDX(i+1,0,dim)].green;
        sum_c_g2 = src[RIDX(i-1,1,dim)].green + src[RIDX(i,1,dim)].green + src[RIDX(i+1,1,dim)].green;

        sum_c_b1 = src[RIDX(i-1,0,dim)].blue + src[RIDX(i,0,dim)].blue + src[RIDX(i+1,0,dim)].blue;
        sum_c_b2 = src[RIDX(i-1,1,dim)].blue + src[RIDX(i,1,dim)].blue + src[RIDX(i+1,1,dim)].blue;



        for (j = 1; j < dim - 1; j+=2) {

            col_r1 = sum_c_r1, col_r2 = sum_c_r2, col_r3 = src[RIDX(i-1,j+1,dim)].red + src[RIDX(i,j+1,dim)].red + src[RIDX(i+1,j+1,dim)].red;
            col_g1 = sum_c_g1, col_g2 = sum_c_g2, col_g3 = src[RIDX(i-1,j+1,dim)].green + src[RIDX(i,j+1,dim)].green + src[RIDX(i+1,j+1,dim)].green;
            col_b1 = sum_c_b1, col_b2 = sum_c_b2, col_b3 = src[RIDX(i-1,j+1,dim)].blue + src[RIDX(i,j+1,dim)].blue + src[RIDX(i+1,j+1,dim)].blue;
            dst[tmp+j].red = (col_r1 + col_r2 + col_r3) / 9;
            dst[tmp+j].green = (col_g1 + col_g2 + col_g3) / 9;
            dst[tmp+j].blue = (col_b1 + col_b2 + col_b3) / 9;
            sum_c_r1 = col_r2, sum_c_r2 = col_r3;
            sum_c_g1 = col_g2, sum_c_g2 = col_g3;
            sum_c_b1 = col_b2, sum_c_b2 = col_b3;

            col_r1 = sum_c_r1, col_r2 = sum_c_r2, col_r3 = src[RIDX(i-1,j+2,dim)].red + src[RIDX(i,j+2,dim)].red + src[RIDX(i+1,j+2,dim)].red;
            col_g1 = sum_c_g1, col_g2 = sum_c_g2, col_g3 = src[RIDX(i-1,j+2,dim)].green + src[RIDX(i,j+2,dim)].green + src[RIDX(i+1,j+2,dim)].green;
            col_b1 = sum_c_b1, col_b2 = sum_c_b2, col_b3 = src[RIDX(i-1,j+2,dim)].blue + src[RIDX(i,j+2,dim)].blue + src[RIDX(i+1,j+2,dim)].blue;
            dst[tmp+j+1].red = (col_r1 + col_r2 + col_r3) / 9;
            dst[tmp+j+1].green = (col_g1 + col_g2 + col_g3) / 9;
            dst[tmp+j+1].blue = (col_b1 + col_b2 + col_b3) / 9;
            sum_c_r1 = col_r2, sum_c_r2 = col_r3;
            sum_c_g1 = col_g2, sum_c_g2 = col_g3;
            sum_c_b1 = col_b2, sum_c_b2 = col_b3;
